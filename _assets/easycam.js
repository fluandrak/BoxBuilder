// let camReset = false; // Ulf 2022-02-06  - declared in sketch.js




// DIVISION WITH PIXEL DENSITY DISABLED (~ROW: 222) - 22-02-06
...
class EasyCam {
  ...
  constructor(renderer, args) {
    ...
    this.mouse = {
      ...
      updateInput : function(x,y,z){
        ...
        // ***********************************************************
        // ***********************************************************
        // *********** Ulf inaktiverat '/pd' på slutet 22-02-06 *****
      ...
      dblclick : function(event){
        ...
        if(cam.mouse.insideViewport(event.x - offX, event.y - offY)){
          ...
          // ***************************************************
          // ***************************************************
          camReset = millis(); // Ulf tillagt 2022-02-06
          // ***************************************************
          // ***************************************************
          // ***************************************************
          // ***************************************************
  ...
  mouseDragRotate() {
    ...
    if (cam.DRAG_CONSTRAINT & cam.AXIS.YAW) {
      cam.dampedRotY.addForce(+dx * (0.8 - myNdc * myNdc));
    }
    if (cam.DRAG_CONSTRAINT & cam.AXIS.PITCH) {
      cam.dampedRotX.addForce(-dy * (0.8 - mxNdc * mxNdc));
    }
...
class Interpolation {
  ...
  constructor(cb){
    ...
    // *********************************************************
    // *********************************************************
    this.default_duration = CAM_TRANSITION_TIME*0.5; // Ulf 22-02-06
    // *********************************************************
    // *********************************************************
    // *********************************************************
    // *********************************************************
...
var Rotation = 
{
  ...
  applyToVec3 : function(rot, vec, dst) {
    ...
    // dst[0] = 2 * (q0 * (x * q0 - (q2 * z - q3 * y)) + s * q1) - x; //ORIGINAL
    
    dst[0] = 2 * (q0 * (x * q0 - (q2 * z - q3 * y*0)) + s * q1) - x; // LAGT TILL *0 efter '...(q2 * z - q3 * y'
    
    // dst[1] = 2 * (q0 * (y * q0 - (q3 * x - q1 * z)) + s * q2) - y; //ORIGINAL
    
    dst[1] = 2 * (q0 * (y * q0 - (q3 * x - q1 * z)) + s * q2) - y*0; // LAGT TILL *0 efter '...+ s * q2) - y'
